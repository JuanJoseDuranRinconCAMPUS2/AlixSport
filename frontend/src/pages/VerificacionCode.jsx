...
export default function VerificationCode({ email, onBackToEmail, onVerificationSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(`${API}/cambiarContrasenia`, 
        {
          "email": email,
          "codigo": code,
    ...
    >
      ...
      >
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-gray-400 text-sm text-center mb-2">Verificación de código</p>
          <p className="text-gray-500 text-xs text-center">Código enviado a: {email}</p>
        </div>

        <motion.form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-gray-300 font-medium mb-1">Código de verificación</label>
            <input
              type="text"
              value={code}
          ...
          <div>
            <label className="block text-gray-300 font-medium mb-1">Nueva contraseña</label>
            <input
              type="password"
              value={newPassword}
          ...
          <div>
            <label className="block text-gray-300 font-medium mb-1">Confirmar contraseña</label>
            <input
              type="password"
          ...
          >
            {loading ? 'Procesando...' : 'Verificar y Cambiar Contraseña'}
          </motion.button>

          <p className="text-center text-sm text-gray-400">
            <button onClick={onBackToEmail} className="text-blue-500 hover:underline">
              Volver atrás
            </button>
          </p>
        </motion.form>
